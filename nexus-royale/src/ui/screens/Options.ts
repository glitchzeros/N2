...
import { setUiScale, getUiScale } from '@/config/experience/Settings';

const STORAGE_KEY = 'nr.accessibility.v1';

type SavedOpts = { highContrast: boolean; colorBlind: 'none' | 'protanopia' | 'deuteranopia' | 'tritanopia'; reduceMotion: boolean; showTracers: boolean; uiScale: number };
...
export class OptionsPanel {
  ...
  mount(parent: HTMLElement = document.body): void {
    ...
    const saved = this.storage.get<SavedOpts | null>(STORAGE_KEY, null) ?? { highContrast: false, colorBlind: 'none', reduceMotion: false, showTracers: true, uiScale: 1 };
    ...
    ui.style.marginLeft = '12px';
    const uiSlider = document.createElement('input'); uiSlider.type = 'range'; uiSlider.min = '0.5'; uiSlider.max = '2'; uiSlider.step = '0.1'; uiSlider.value = String(saved.uiScale);
    ui.append(document.createTextNode('UI Scale '), uiSlider);

    el.append(hc, cb, rm, tr, ui);
    ...
    const persist = () => this.storage.set(STORAGE_KEY, { highContrast: hcCb.checked, colorBlind: sel.value as SavedOpts['colorBlind'], reduceMotion: rmCb.checked, showTracers: trCb.checked, uiScale: parseFloat(uiSlider.value) });
    ...
    uiSlider.addEventListener('input', () => { setUiScale(parseFloat(uiSlider.value)); persist(); });
  }
